...
function createGateClient(credentials){
  ...
  async function optionalFutures(label,work,warnings){
    try{return await work();}catch(error){
      if(/please transfer funds first to create futures account/i.test(String(error?.message||'')))return null;
      return optional(label,()=>Promise.reject(error),warnings);
    }
  }
  async function fetchAccounts(){
    const warnings=[],spot=await request('/api/v4/spot/accounts');
    const results=await Promise.all(['usdt','btc'].map(settle=>Promise.all([optionalFutures(`${settle.toUpperCase()} futures account`,()=>request(`/api/v4/futures/${settle}/accounts`),warnings),optionalFutures(`${settle.toUpperCase()} futures positions`,()=>request(`/api/v4/futures/${settle}/positions`),warnings)])));

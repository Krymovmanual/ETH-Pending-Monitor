(() => {
  ...
  async function refresh(){
    ...
    for(const c of connections.items)row($('connections'),c.name,`${String(c.exchange||'').toUpperCase()} · ${c.key_hint} · ${c.status}${c.last_checked?' · '+new Date(c.last_checked).toLocaleString():''}`,{label:'Remove',run:()=>{$('removeConnection').hidden=false;$('removeConnection').elements.id.value=c.id;$('removeName').textContent=`Remove ${c.name}?`;}});
